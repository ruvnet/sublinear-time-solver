...
// Test script to verify WASM functionality in Node.js
...
try {
    ...
    console.log(`   Pairs found: ${result.pairs_found}`);
} catch (error) {
...
try {
    ...
    const trajectory = studio.predict_trajectory(testInput, 5);
    console.log(`✅ Multi-step trajectory prediction: ${trajectory.length / 3} steps`);
} catch (error) {
    console.error('❌ Error:', error);
}
...
console.log('✅ Interpretation generated for λ=0.9');
...
console.log('✅ Parameter recommendations generated');

// Test 9: Regime change detection
console.log('\nTest 9: Testing regime change detection...');
try {
    const regimeChanges = studio.detect_regime_changes(
        lorenzData.slice(0, 600),
        3,
        50,  // window_size
        10   // stride
    );
    console.log(`✅ Detected ${regimeChanges.length} regime windows`);
} catch (error) {
    console.error('❌ Error:', error);
}
...
console.log('\n📊 Summary: The WASM build provides full chaos analysis capabilities!');