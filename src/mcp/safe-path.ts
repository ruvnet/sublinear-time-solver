...
export const DEFAULT_VECTOR_DIR =
  process.env.SUBLINEAR_SOLVER_VECTOR_DIR ||
...
export class SafePathError extends Error {
  ...
  constructor(message: string, code: string = 'EUNSAFE_PATH') {
    super(message);
...
export function assertSafeBasename(name: unknown): asserts name is string {
  ...
  if (name.includes('/') || name.includes('\\')) {
    throw new SafePathError(
      `file_path must be a basename without path separators (got "${name}")`,
      'EPATHSEP',
    );
  }
  if (name === '.' || name === '..') {
  ...
  if (name.startsWith('.')) {
    throw new SafePathError(
      `file_path must not start with "." (got "${name}")`,
      'EHIDDEN',
    );
  }
  for (let i = 0; i < name.length; i++) {
    ...
    if (c < 0x20 || c === 0x7f) {
      throw new SafePathError(
        `file_path contains control character (0x${c.toString(16)})`,
        'ECONTROL',
      );
    }
  }
  ...
  if (WINDOWS_RESERVED.has(stem)) {
    throw new SafePathError(
      `file_path uses Windows-reserved device name "${stem}"`,
      'ERESERVED',
    );
  }
}
...
export function resolveVectorPath(
  ...
  if (rel.startsWith('..') || path.isAbsolute(rel) || rel.includes(`..${path.sep}`)) {
    throw new SafePathError(
      `resolved path "${candidate}" escapes vector dir "${baseAbs}"`,
      'EOUTSIDE',
    );
  }
  return candidate;
...
export function openSafeWriteFd(absPath: string): number {
  const flags =
    fs.constants.O_WRONLY |
    fs.constants.O_CREAT |
    fs.constants.O_TRUNC |
    ((fs.constants as any).O_NOFOLLOW || 0) |
    ((fs.constants as any).O_CLOEXEC || 0);
  return fs.openSync(absPath, flags, 0o600);
}

export function openSafeReadFd(absPath: string): number {
  const flags =
...
export function safeWriteVector(
  filename: unknown,
  data: string | Buffer,
  options?: { stateDir?: string },
): string {
...
export function safeReadVector(
  filename: unknown,
  options?: { stateDir?: string },
): string {
  const abs = resolveVectorPath(filename, options);
  ...
  try {
    ...
    if (!stat.isFile()) {
      throw new SafePathError(
        `vector path "${abs}" is not a regular file`,
        'ENOTFILE',
      );
    }
