...
export class SolverTools {
  ...
  static async solve(params: SolveParams) {
    ...
    try {
      ...
      throw new SolverError(
        `Solve operation failed: ${error instanceof Error ? error.message : String(error)}`,
        'INTERNAL_ERROR',
        { originalError: error }
      );
    }
   ...
   */
  static async estimateEntry(params: EstimateEntryParams) {
    try {
      // Enhanced validation
      if (!params.matrix) {
        throw new SolverError('Matrix parameter is required', 'INVALID_PARAMETERS');
      ...
      throw new SolverError(
        `Entry estimation failed: ${error instanceof Error ? error.message : String(error)}`,
        'INTERNAL_ERROR',
        {
          row: params.row,
          column: params.column,
   ...
   */
  static async *streamingSolve(params: SolveParams, progressCallback?: (progress: any) => void) {
    const config: SolverConfig = {
      method: params.method || 'neumann',
      epsilon: params.epsilon || 1e-6,
   ...
   */
  static async batchSolve(matrix: any, vectors: number[][], params: Partial<SolveParams> = {}) {
    const config: SolverConfig = {
      method: params.method || 'neumann',
      epsilon: params.epsilon || 1e-6,
      maxIterations: params.maxIterations || 1000,
