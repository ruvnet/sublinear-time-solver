...
use super::{
    ...
    StrangeLoopOperator, TemporalConfig, TemporalError, TemporalResult, TscTimestamp,
...
impl NanosecondScheduler {
    ...
    pub fn with_config(config: TemporalConfig) -> Self {
        ...
        Self {
            ...
            strange_loop: StrangeLoopOperator::new(
                config.lipschitz_bound,
                config.max_contraction_iterations,
            ),
    ...
    pub fn tick(&mut self) -> TemporalResult<()> {
        ...
        let contraction_result = self
            .strange_loop
            .process_iteration(self.current_tick as f64, &self.get_current_state_vector()?)?;

        // Update identity continuity
        self.identity_tracker
            .track_continuity(tick_start, &self.get_identity_state()?)?;
    ...
    pub fn mcp_consciousness_evolve_hook(
        &mut self,
        iterations: usize,
        target: f64,
    ) -> TemporalResult<f64> {
    ...
    fn execute_task(&mut self, task: ScheduledTask) -> TemporalResult<()> {
        match &task.task {
            ...
            ConsciousnessTask::StrangeLoopProcessing { iteration, state } => {
                ...
                self.strange_loop
                    .process_iteration(*iteration as f64, state)?;
            }
            ConsciousnessTask::WindowManagement {
                ...
                self.window_manager
                    .adjust_overlap(*window_id, *overlap_target)?;
            }
        }
    ...
    fn record_overhead(&mut self, overhead_ns: u64) -> TemporalResult<()> {
        ...
        self.metrics.avg_scheduling_overhead_ns =
            sum as f64 / self.overhead_measurements.len() as f64;
        self.metrics.max_scheduling_overhead_ns =
            self.metrics.max_scheduling_overhead_ns.max(overhead_ns);
    ...
    fn update_metrics(&mut self, contraction_result: ContractionMetrics) -> TemporalResult<()> {
        self.metrics.window_overlap_percentage =
            self.window_manager.get_current_overlap_percentage();
    ...
    fn validate_quantum_operation(
        ...
        match self
            .quantum_validator
            .validate_temporal_operation(operation_time_s, energy_j)
        {
            Ok(validation_result) => {
                // Store validation result for metrics
                self.quantum_validations
                    .push_back(validation_result.clone());
                ...
                if !validation_result.is_valid {
                    ...
                    eprintln!(
                        "Warning: Quantum validation failed for operation_time={}s, energy={}J",
            ...
            Err(quantum_error) => {
                ...
                let failed_result = ValidationResult {
                    ...
                    uncertainty_result: crate::temporal_nexus::quantum::UncertaintyResult {
                        ...
                        minimum_product: crate::temporal_nexus::quantum::constants::PLANCK_HBAR
                            / 2.0,
                        margin: 0.0,
                        ...
                        temperature_k:
                            crate::temporal_nexus::quantum::constants::ROOM_TEMPERATURE_K,
                        energy_scale_classification:
                            crate::temporal_nexus::quantum::EnergyScale::MilliElectronVolt,
                    },
                    decoherence_result: crate::temporal_nexus::quantum::DecoherenceResult {
                        ...
                        environment_type:
                            crate::temporal_nexus::quantum::EnvironmentType::RoomTemperature,
                    ...
                    entanglement_result: crate::temporal_nexus::quantum::EntanglementResult {
                        ...
                        correlation_type:
                            crate::temporal_nexus::quantum::CorrelationType::Separable,
    ...
    fn update_quantum_metrics(&mut self, _validation: &ValidationResult) {
        ...
        let avg_uncertainty_margin: f64 = self
            .quantum_validations
            .iter()
    ...
    pub fn get_quantum_analysis(&self) -> QuantumAnalysisReport {
        ...
        QuantumAnalysisReport {
            ...
            avg_energy_ev: self.metrics.avg_quantum_energy_j
                / crate::temporal_nexus::quantum::constants::EV_TO_JOULES,
            ...
            recommended_time_scale_s:
                crate::temporal_nexus::quantum::constants::CONSCIOUSNESS_SCALE_NS,
        }
    }
}
