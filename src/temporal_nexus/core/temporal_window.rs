...
impl WindowOverlapManager {
    ...
    pub fn adjust_overlap(
        &mut self,
        window_id: u64,
        new_overlap_target: f64,
    ) -> TemporalResult<()> {
        for window in &mut self.windows {
            if window.info.window_id == window_id {
                let new_overlap_size =
                    ((window.size() as f64) * (new_overlap_target / 100.0)) as u64;
    ...
    fn should_create_new_window(&self) -> TemporalResult<bool> {
        ...
        let window_progress = (self.current_tick - current_window.info.start_tick) as f64
            / current_window.size() as f64;
    ...
    fn create_new_window(&mut self) -> TemporalResult<()> {
        ...
        let (start_tick, overlap_size) = if let Some(prev_window) = self.windows.back() {
            ...
            let overlap_ticks =
                ((self.window_size_ticks as f64) * (self.target_overlap_percent / 100.0)) as u64;
    ...
    fn update_metrics(&mut self) {
        ...
        if !overlap_percentages.is_empty() {
            self.metrics.average_overlap_percentage =
                overlap_percentages.iter().sum::<f64>() / overlap_percentages.len() as f64;
            self.metrics.min_overlap_percentage = overlap_percentages
                .iter()
                .cloned()
                .fold(f64::INFINITY, f64::min);
            self.metrics.max_overlap_percentage = overlap_percentages
                .iter()
                .cloned()
                .fold(f64::NEG_INFINITY, f64::max);

            // Calculate optimal overlap ratio (how close we are to target)
            self.metrics.optimal_overlap_ratio =
                self.metrics.average_overlap_percentage / self.target_overlap_percent;
        }
    }
    ...
    fn recalculate_window_boundaries(&mut self, window_id: u64) -> TemporalResult<()> {
        ...
        for i in 0..self.windows.len() {
            if self.windows[i].info.window_id == window_id {
                ...
                // Adjust end time
                self.windows[i].info.end_tick =
                    self.windows[i].info.start_tick + self.window_size_ticks - 1;

                // Adjust next window if exists
                if i + 1 < self.windows.len() {
                    let next_overlap = self.windows[i + 1].info.overlap_size;
                    self.windows[i + 1].info.start_tick =
                        self.windows[i].info.end_tick - next_overlap + 1;
                }
