...
impl StrangeLoopOperator {
    ...
    pub fn process_iteration(
        ...
        time: f64,
        state: &[f64],
    ) -> TemporalResult<ContractionMetrics> {
        ...
        for prev_state in self.loop_states.iter().rev() {
            if prev_state.state_vector.len() == state.len() {
                let correlation = self
                    .calculate_correlation(&prev_state.state_vector, state)
                    .unwrap_or(0.0);
    ...
    fn calculate_state_complexity(&self, state: &[f64]) -> f64 {
        ...
        let variance: f64 =
            state.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / state.len() as f64;
    ...
    fn update_self_reference(&mut self, loop_state: &LoopState) -> TemporalResult<()> {
        ...
        self.self_reference_strength =
            (1.0 - alpha) * self.self_reference_strength + alpha * loop_state.self_reference;
    ...
    fn update_metrics(&mut self, convergence_info: ConvergenceInfo) -> TemporalResult<()> {
        ...
        if self.contraction_history.len() >= 2 {
            let recent_distances: Vec<f64> = self
                .contraction_history
                .iter()
                .rev()
                .take(10)
            ...
            if recent_distances.len() >= 2 {
                let rate =
                    recent_distances[0] / recent_distances[recent_distances.len() - 1].max(1e-10);
    ...
    fn calculate_stability_measure(&self) -> f64 {
        ...
        let variance: f64 = recent_distances
            .iter()
            .map(|x| (x - mean).powi(2))
            .sum::<f64>()
